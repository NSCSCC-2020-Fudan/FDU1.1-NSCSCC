...
#
# NOTE: "__" is reserved for this script. DO NOT USE DOUBLE UNDERLINES
#       IN YOUR RTL DESIGNS.
#       dots in hierarchical references will be replace by "__"
#
# Usage:
...
def dump(path: Text):
    ...
    for t in type_table:
        ...
        if t.tag == 'basicdtype':
            ...
            assert name is not None
            name = removeprefix(name, '$unit::').replace('.', '__')
            type_mp[idx] = name
...
def parse(name: Text):
    ...
    for obj in assigns:
        ...
        obj.referee = f'__top.{obj.referee}'
    ...
    for port in ports:  # incrimental expansion
        ...
        for member in struct.members:
            new_name = f'{port.name}__{member.name}'
            ports.append(Port(
